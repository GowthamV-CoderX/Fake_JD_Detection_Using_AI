...
====================
Location in your repo: feature_extractor.py  (root)
...
CAPS_WORD_PATTERN = re.compile(r"\b[A-Z]{3,}\b")
...
class FeatureExtractor:
    """Extracts ~24 structured features from a job description string."""

    def extract(
        self, text: str, title: str = "", company: str = ""
    ) -> ExtractedFeatures:
        ...
        feat.avg_word_length    = sum(len(w) for w in words) / len(words) if words else 0.0
        ...
        feat.has_domain_mention  = bool(
            re.search(r"\.(com|in|org|net|co)\b", full_text, re.IGNORECASE)
        )
    ...
    def _compute_spam_score(self, f: ExtractedFeatures) -> float:
        score = 0.0
        score += min(f.exclamation_count / 4.0, 1.0)         * 0.18
        score += min(f.emoji_count / 3.0, 1.0)               * 0.12
        score += min(f.caps_word_count / 8.0, 1.0)           * 0.12
        score += min(f.urgency_phrase_count / 2.0, 1.0)      * 0.20
        score += min(f.vague_promise_count / 2.0, 1.0)       * 0.18
        score += min(f.payment_request_count / 1.0, 1.0)     * 0.25
        score += (1.0 if f.has_unrealistic_salary else 0.0)  * 0.15
        score += (
            1.0 if f.has_generic_email and not f.has_company_email else 0.0
        ) * 0.12
        score += min(f.network_marketing_signals / 1.0, 1.0) * 0.08
        score += (1.0 if f.whatsapp_mentions > 0 else 0.0)   * 0.10
