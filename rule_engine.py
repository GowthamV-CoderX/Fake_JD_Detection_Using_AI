...
class RuleEngine:
    ...
    def evaluate(self, features, text: str) -> RuleEngineOutput:
        ...
        if ("generic_remote" in matched and
            "no_experience" in matched and
            "high_salary" in matched):
            score += 1.2
            triggered_flags.append("combo_remote_highpay")

        if ("external_contact" in matched and
            "urgency" in matched):
            score += 1.0
            triggered_flags.append("combo_pressure")

        if ("ai_vague" in matched and
            "no_structure" in matched):
            score += 1.0
            triggered_flags.append("combo_ai_fake")

        if re.search(r"(earn based on|per click|per engagement|referral)", text):
            score += 1.0
        ...
        if not re.search(r"(python|java|sql|excel|aws|react)", text):
            score += 0.3
            triggered_flags.append("no_skills")

        if not re.search(r"(inc|ltd|corp|technologies|solutions)", text):
            score += 0.2
